...
columns_to_load = [
    'Start Date', 'End Date', 'Start Time Zone', 'End Time Zone',
    'Energy (kWh)', 'Postal Code', 'Station Name', 'City'
]

df = pd.read_csv(
    '../../data/data_raw/raw_palo_alto/ChargePoint Data CY20Q4.csv',
    usecols=columns_to_load
)


# =========================================================
# PREPROCESS DATA (basic sanity fixes)
# =========================================================
# QA counters (not printed)
missing_values = df.isnull().sum()
negative_energy_count = (df['Energy (kWh)'] < 0).sum()
_ = (df['Start Time Zone'] != df['End Time Zone']).sum()  # timezone label mismatch count

# Normalize to timezone-aware UTC; coerce invalid to NaT (keeps behavior)
df = df.copy()
...
def expand_sessions_to_intervals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Returns a DataFrame with one row per 10-minute slice:
      ['Timestamp', 'Energy (kWh)', 'Station Name', 'City', 'Postal Code']
    """
    # Duration of each session in 10-minute steps (integer via floor division)
    durations = ((df['End Date'] - df['Start Date']).dt.total_seconds() // 600)

    # Pre-allocate arrays for speed (same semantics as original)
    total_rows = int(durations.sum())
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        energy_per_interval = row['Energy (kWh)'] / duration
        energy_values[idx:end_idx] = energy_per_interval

        # Copy session metadata to each expanded row
        station_names[idx:end_idx] = row['Station Name']
        cities[idx:end_idx]        = row['City']
        regions[idx:end_idx]       = row['Postal Code']
    ...
    return pd.DataFrame({
        ...
        'Energy (kWh)': energy_values[:idx],
        'Station Name': station_names[:idx],
        'City': cities[:idx],
        'Postal Code': regions[:idx]
    })
...
df = df.rename(columns={"Energy (kWh)": "Total kWh", "Station Name": "Station_Name"})
...
def build_holidays(idx: pd.DatetimeIndex, us_holidays=None) -> pd.DatetimeIndex:
    """
    Return a DatetimeIndex of California holiday dates (normalized to midnight)
    ...
    if us_holidays is None:
        ...
        us_holidays = holidays.US(state='CA', years=years)
...
# Fixed holiday set (California), years as provided
us_holidays = holidays.US(state='CA', years=range(2011, 2021))
...
# SAVE RESULTS
# =========================================================
save_path = os.path.join('..', '..', 'data', 'data_preprocessed', 'preprocessed_palo_alto')
