...
columns_to_load = [
    'Station_Name', 'City', 'Zip_Postal_Code',
    'Start_Date___Time', 'Start_Time_Zone',
    'End_Date___Time', 'End_Time_Zone',
    'Energy__kWh_'
]

df = pd.read_csv(
    '../../data/data_raw/raw_boulder/ev_chargingstationdata_Boulder_March_2021.csv',
...
# PREPROCESS DATA
# =========================================================
# Count missing values (for reference/QA; not printed)
missing_values = df.isnull().sum()

# Count negative energy values (for reference/QA; not printed)
negative_energy_count = (df['Energy__kWh_'] < 0).sum()

# Drop sessions where end < start (invalid durations)
df = df[df['End_Date___Time'] > df['Start_Date___Time']]

# Count rows where timezone labels differ (for reference/QA; not printed)
_ = (df['Start_Time_Zone'] != df['End_Time_Zone']).sum()

# Convert timestamps to timezone-aware UTC (consistent time base)
df = df.copy()
df['Start_Date___Time'] = pd.to_datetime(df['Start_Date___Time'], utc=True)
df['End_Date___Time']   = pd.to_datetime(df['End_Date___Time'],   utc=True)
...
def expand_sessions_to_intervals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    the total session energy across its intervals.

    Returns a DataFrame with one row per 10-minute slice:
      ['Timestamp', 'Energy__kWh_', 'Station_Name', 'City', 'Zip_Postal_Code']
    """
    # Duration of each session in 10-minute steps (integer)
    durations = ((df['End_Date___Time'] - df['Start_Date___Time'])
                 .dt.total_seconds() / 600).astype(int)

    # Pre-allocate arrays for speed (exact same logic as original)
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        # 10-min spaced timestamps across the session length
        ...
        energy_per_interval = row['Energy__kWh_'] / duration
        energy_values[idx:end_idx] = energy_per_interval

        # Copy session metadata for each expanded row
        station_names[idx:end_idx] = row['Station_Name']
        cities[idx:end_idx]        = row['City']
        regions[idx:end_idx]       = row['Zip_Postal_Code']

        idx = end_idx

    # Build the expanded per-interval DataFrame (trim to filled length)
    return pd.DataFrame({
        'Timestamp': pd.to_datetime(timestamps[:idx]),
        'Energy__kWh_': energy_values[:idx],
        'Station_Name': station_names[:idx],
        'City': cities[:idx],
        'Zip_Postal_Code': regions[:idx]
    })


df = expand_sessions_to_intervals(df)
df = df.rename(columns={"Energy__kWh_": "Total kWh"})
...
def make_10min_grid_per_station(
    df: pd.DataFrame,
    target_in: str = 'Energy__kWh_',
    target_out: str = 'Total kWh',
    region_col: str = 'Zip_Postal_Code'
) -> pd.DataFrame:
    """
    ...
    # Per-station/region, sum into 10-min bins and make grid dense
    s_full = (
...
df_10min = make_10min_grid_per_station(df, 'Total kWh', 'Total kWh')
...
def build_holidays(idx: pd.DatetimeIndex, us_holidays=None) -> pd.DatetimeIndex:
    """
    Return a set-like DatetimeIndex of holiday dates (normalized to midnight)
    covering the years present in idx. If us_holidays is given, reuse it.
    """
    if us_holidays is None:
        years = range(int(idx.year.min()), int(idx.year.max()) + 1)
        us_holidays = holidays.US(state='CO', years=years)
...
# Fixed holiday set (same years as before; CO-specific)
us_holidays = holidays.US(state='CO', years=range(2018, 2022))
...
# SAVE RESULTS
# =========================================================
save_path = os.path.join('..', '..', 'data', 'data_preprocessed', 'preprocessed_boulder')
