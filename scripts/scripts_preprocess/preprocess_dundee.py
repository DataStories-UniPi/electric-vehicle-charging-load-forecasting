...
columns_to_load = [
    'CP ID', 'Start Date', 'Start Time', 'End Date', 'End Time', 'Total kWh', 'Site'
]

df1 = pd.read_csv('../../data/data_raw/raw_dundee/Charging Sessions Dundee 010917 - 011217.csv',
                  usecols=columns_to_load)
df2 = pd.read_csv('../../data/data_raw/raw_dundee/Charging Sessions Dundee 021217- 020318.csv',
                  usecols=columns_to_load)
df3 = pd.read_csv('../../data/data_raw/raw_dundee/Charging Sessions Dundee 050318 - 050618.csv',
                  usecols=columns_to_load)
df4 = pd.read_csv('../../data/data_raw/raw_dundee/Charging Sessions Dundee 060618 - 060918.csv',
                  usecols=columns_to_load)
...
# First 3 files have the same format
df1 = process_df(df1)
df2 = process_df(df2)
df3 = process_df(df3)
...
# df4 uses day-first parsing; align its columns and append
df4 = df4.copy()
df4['Start Date'] = pd.to_datetime(df4['Start Date'] + ' ' + df4['Start Time'], dayfirst=True)
df4['End Date']   = pd.to_datetime(df4['End Date']   + ' ' + df4['End Time'],   dayfirst=True)
df4 = df4[df.columns.intersection(df4.columns)]
...
# Drop raw time columns; we’ll carry the combined datetimes
df = df.drop(columns=['Start Time', 'End Time'])

# Normalize to timezone-aware UTC
df = df.copy()
df['Start Date'] = pd.to_datetime(df['Start Date'], utc=True)
df['End Date']   = pd.to_datetime(df['End Date'],   utc=True)
...
def build_holidays(idx: pd.DatetimeIndex, us_holidays=None) -> pd.DatetimeIndex:
    """
    Return a DatetimeIndex of Scottish holiday dates (normalized to midnight)
    ...
    if us_holidays is None:
        ...
        us_holidays = holidays.country_holidays('GB', subdiv='SCT', years=years)
...
# Fixed holiday set for Scotland (matches original intent: 2017–2018)
us_holidays = holidays.country_holidays('GB', subdiv='SCT', years=[2017, 2018])
...
# SAVE RESULTS
# =========================================================
save_path = os.path.join('..', '..', 'data', 'data_preprocessed', 'preprocessed_dundee')
